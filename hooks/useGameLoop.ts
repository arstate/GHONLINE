...
export function useGameLoop({
  ...
  const activeHoldsP1Ref = useRef<(Note | null)[]>([
    null,
    null,
    null,
    null,
  ...
  const activeHoldsP2Ref = useRef<(Note | null)[]>([
    null,
    null,
    null,
    null,
  ...
  const syncScoreUI = useCallback(() => {
    if (!cachedScore1Ref.current)
      cachedScore1Ref.current = document.getElementById("scoreDisplay");
    if (!cachedScore2Ref.current)
      cachedScore2Ref.current = document.getElementById("scoreDisplayGame");

    if (gameMode === "multiplayer") {
      const p1Str = scoreP1Ref.current.toString().padStart(6, "0");
      const p2Str = scoreP2Ref.current.toString().padStart(6, "0");
      if (cachedScore1Ref.current)
        cachedScore1Ref.current.textContent = `${p1Str} | ${p2Str}`;
      if (cachedScore2Ref.current)
        cachedScore2Ref.current.textContent = `${p1Str} | ${p2Str}`;
    } else {
  ...
  const triggerHitP1 = useCallback(
    (laneIndex: number) => {
  ...
  const triggerHitP2 = useCallback(
    (laneIndex: number) => {
  ...
  useEffect(() => {
    ...
    const getScale = (z: number) =>
      Math.max(
        0.01,
        FOCAL_LENGTH / (FOCAL_LENGTH + Math.max(z, -FOCAL_LENGTH + 10)),
      );
    const getScreenX = (offsetX: number, z: number, vpX: number) =>
      vpX + offsetX * getScale(z);
    ...
    const getCachedPuck = (color: string, missed: boolean, isHit: boolean) => {
      ...
      const bRX = baseSize * drawScale,
        bRY = baseSize * 0.36 * drawScale,
        iBRX = baseSize * 0.9 * drawScale,
        iBRY = baseSize * 0.3 * drawScale,
        tRX = baseSize * 0.5 * drawScale,
        tRY = baseSize * 0.18 * drawScale,
        nH = baseSize * 0.4 * drawScale;
      const tY = cy - nH,
        nC = missed ? "#64748b" : color;

      // Base glow if hit
      if (isHit) {
        ctx.fillStyle = color + "44";
        ctx.beginPath();
        ctx.ellipse(cx, cy, bRX * 2, bRY * 2, 0, 0, Math.PI * 2);
        ctx.fill();
        ctx.fillStyle = color + "22";
        ctx.beginPath();
        ctx.ellipse(cx, cy, bRX * 2.5, bRY * 2.5, 0, 0, Math.PI * 2);
        ctx.fill();
      }
      ...
      ctx.fill();
      const grad = ctx.createLinearGradient(cx - iBRX, 0, cx + iBRX, 0);
      grad.addColorStop(0, "#000");
      grad.addColorStop(0.3, nC);
      grad.addColorStop(0.7, nC);
      grad.addColorStop(1, "#000");
      ctx.fillStyle = grad;
      ctx.beginPath();
      ctx.ellipse(cx, cy, iBRX, iBRY, 0, 0, Math.PI * 2);
      ctx.fill();
      ctx.beginPath();
      ctx.moveTo(cx - iBRX, cy);
      ctx.lineTo(cx + iBRX, cy);
      ctx.lineTo(cx + tRX, tY);
      ctx.lineTo(cx - tRX, tY);
      ctx.fill();
      ctx.fillStyle = nC;
      ctx.beginPath();
      ctx.ellipse(cx, tY, tRX, tRY, 0, 0, Math.PI * 2);
      ctx.fill();
      ...
      ctx.beginPath();
      ctx.ellipse(cx, tY, btRX, btRY, 0, 0, Math.PI * 2);
      ctx.fill();
      ctx.beginPath();
      ctx.moveTo(cx - btRX, tY);
      ctx.lineTo(cx + btRX, tY);
      ctx.lineTo(cx + btRX, btTY);
      ctx.lineTo(cx - btRX, btTY);
      ctx.fill();
      ...
      if (!missed) {
        ...
        ctx.beginPath();
        ctx.ellipse(cx, btTY, btRX * 1.5, btRY * 1.5, 0, 0, Math.PI * 2);
        ctx.fill();
        if (isHit) {
          ctx.fillStyle = "rgba(255, 255, 255, 0.4)";
          ctx.beginPath();
          ctx.ellipse(cx, btTY, btRX * 2, btRY * 2, 0, 0, Math.PI * 2);
          ctx.fill();
        }
      }
      ctx.fillStyle = "#fff";
      ctx.beginPath();
      ctx.ellipse(cx, btTY, btRX, btRY, 0, 0, Math.PI * 2);
      ctx.fill();
    ...
    const loop = (time: number) => {
      ...
      for (const gp of gamepads) {
        ...
        gp.buttons.forEach((btn, idx) => {
          ...
          const p1Lane = (p1Keys as string[]).findIndex(
            (m) => m.toLowerCase() === mapping,
          );
          const p2Lane = (p2Keys as string[]).findIndex(
            (m) => m.toLowerCase() === mapping,
          );

          if (p1Lane !== -1) {
            if (btn.pressed && !lastGamepadState.current[idx])
              triggerHitP1(p1Lane);
            else if (!btn.pressed && lastGamepadState.current[idx])
              triggerReleaseP1(p1Lane);
          }
          if (p2Lane !== -1) {
            if (btn.pressed && !lastGamepadState.current[idx])
              triggerHitP2(p2Lane);
            else if (!btn.pressed && lastGamepadState.current[idx])
              triggerReleaseP2(p2Lane);
          }
      ...
      if (startGameRequestRef.current) {
        ...
        if (audioCtxRef.current && audioBufferRef.current) {
          ...
          const speedFactor =
            difficulty === "hard" ? 1.5 : difficulty === "extreme" ? 2.0 : 1.0;
      ...
      if (gameStateRef.current === "game") {
        ...
        if (!ts2Cached && gameMode === "multiplayer")
          ts2Cached = document.getElementById("trackSurfaceP2");
        if (ts1Cached)
          ts1Cached.style.backgroundPositionY = `${trackScrollYRef.current}px`;
        if (ts2Cached)
          ts2Cached.style.backgroundPositionY = `${trackScrollYRef.current}px`;
          ...
          if (audioBufferRef.current) {
            ...
            const duration =
              buf instanceof AudioBuffer
                ? buf.duration
                : (buf as any).vocals.duration;
            if (
              pbTime > duration + 1.0 &&
              spawnedIndexP1Ref.current >= beatsP1.length &&
              notesP1Ref.current.length === 0 &&
              notesP2Ref.current.length === 0
            ) {
        ...
        for (let i = notesP1Ref.current.length - 1; i >= 0; i--) {
          ...
          if (n.isActiveHold) {
            ...
            if ((n.type === "hold" ? n.z + n.lengthZ : n.z) < -300)
              notesP1Ref.current.splice(i, 1);
          }
        }
        ...
        for (let i = 0; i < 5; i++) {
          if (laneHitStatesP1Ref.current[i] > 0)
            laneHitStatesP1Ref.current[i] -= deltaTime;
          if (laneHitStatesP2Ref.current[i] > 0)
            laneHitStatesP2Ref.current[i] -= deltaTime;
        }
      ...
      const renderTrack = (
        ...
        [-1, 1].forEach((side) => {
          ctx.beginPath();
          ctx.moveTo(
            getScreenX(side * sideOff, START_Z, vpX),
            getScreenY(START_Z),
          );
        ...
        for (let i = notes.length - 1; i >= 0; i--) {
          ...
          if (n.type === "hold" && n.lengthZ > 0 && n.z <= START_Z) {
            const bZ = n.z,
              tZ = Math.min(n.z + n.lengthZ, START_Z);
            const wB = (gameMode === "multiplayer" ? 25 : 45) * getScale(bZ),
              wT = (gameMode === "multiplayer" ? 25 : 45) * getScale(tZ);
            const xB = getScreenX(off, bZ, vpX),
              yB = getScreenY(bZ),
              xT = getScreenX(off, tZ, vpX),
              yT = getScreenY(tZ);
      ...
      if (gameMode === "multiplayer") {
        renderTrack(
          notesP1Ref.current,
          width * 0.25,
          activeLanesP1Ref.current,
          activeHoldsP1Ref.current,
          laneHitStatesP1Ref.current,
          p1Keys,
        );
        renderTrack(
          notesP2Ref.current,
          width * 0.75,
          activeLanesP2Ref.current,
          activeHoldsP2Ref.current,
          laneHitStatesP2Ref.current,
          p2Keys,
        );
      } else {
        renderTrack(
    ...
    const handlePointerDown = (e: PointerEvent | TouchEvent) => {
      ...
      const touches =
        "touches" in e ? Array.from(e.changedTouches) : [e as PointerEvent];
      touches.forEach((t) => {
